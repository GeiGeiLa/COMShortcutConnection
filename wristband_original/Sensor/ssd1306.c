...
#define SSD1306_CONFIG_SDA_PIN      6
#define SSD1306_nRES_PIN            4
...
{
#if 0
    if (p_event->type == NRFX_TWI_ERROR) {
        printf("E\r\n");
    }

    if (p_event->type == NRFX_TWI_TX_DONE) {
        printf("T\r\n");
    }

    if (p_event->type == NRFX_TWI_RX_DONE) {
        printf("R\r\n");
    }
#endif
...
ret_code_t twi_master_init(uint32_t scl, uint32_t sda)
{
    ...
    do {
        ...
        //ret = nrf_drv_twi_init(&m_twi_master, &config, twi_evt_handler, NULL);
        if (NRF_SUCCESS != ret) {
            break;
        }
...
void ssd1306_init_i2c(void)
{
    _i2caddr = SSD1306_I2C_ADDRESS;
    twi_master_init(SSD1306_CONFIG_SCL_PIN, SSD1306_CONFIG_SDA_PIN);
}
...
void ssd1306_begin(uint8_t vccstate, uint8_t i2caddr, bool reset)
{
    ...
    ssd1306_command(0xAE);nrf_delay_ms(1);
    ssd1306_command(0xD5);nrf_delay_ms(1);
    ssd1306_command(0xD1);nrf_delay_ms(1);
    ssd1306_command(0xA8);nrf_delay_ms(1);
    ssd1306_command(0x1F);nrf_delay_ms(1);
    ssd1306_command(0xD3);nrf_delay_ms(1);
    ssd1306_command(0x00);nrf_delay_ms(1);
    ssd1306_command(0x40);nrf_delay_ms(1);
    
    ssd1306_command(0x8D);nrf_delay_ms(1);
    ssd1306_command(0x14);nrf_delay_ms(1);
